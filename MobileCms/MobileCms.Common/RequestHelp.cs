...
namespace MobileCms.Common
{
    public class RequestHelp
    {
        ...
        public static string GetIP()
        {
            ...
            string variable = HttpContext.Current.Request.ServerVariables["HTTP_X_FORWARDED_FOR"];
            if (!String.IsNullOrEmpty(variable))
            {
                if (variable.IndexOf("unknown") > 0)
                {
                    result = HttpContext.Current.Request.ServerVariables["REMOTE_ADDR"].Trim();
                }
                else if (variable.IndexOf(",") > 0)
                {
                    result = variable.Substring(0, variable.IndexOf(",")).Trim();
                }
                else if (variable.IndexOf(";") > 0)
                {
                    result = variable.Substring(0, variable.IndexOf(";")).Trim();
